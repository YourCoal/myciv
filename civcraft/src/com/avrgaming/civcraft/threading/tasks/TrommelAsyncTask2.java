...
import com.avrgaming.civcraft.structure.Trommel2;
import com.avrgaming.civcraft.structure.Trommel2.MineralNew;
import com.avrgaming.civcraft.threading.CivAsyncTask;
import com.avrgaming.civcraft.threading.sync.request.UpdateInventoryRequest.Action;
import com.avrgaming.civcraft.util.ItemManager;
import com.avrgaming.civcraft.util.MultiInventory;

public class TrommelAsyncTask2 extends CivAsyncTask {
	...
	public static void debug(Trommel2 trommel, String msg) {
		if (debugTowns.contains(trommel.getTown().getName())) {
			CivLog.warning("TrommelDebug:"+trommel.getTown().getName()+":"+msg);
		}
	}
	
	public TrommelAsyncTask2(Structure trommel) {
		this.trommel = (Trommel2)trommel;
	}
		...
		for (int i = 0; i < trommel.skippedCounter+1; i++) {
			for(ItemStack stack : contents) {
				...
				if (ItemManager.getId(stack) == CivData.COBBLESTONE) {
					...
					int randMax = Trommel2.MAX_CHANCE;
					int rand1 = rand.nextInt(randMax);
					ItemStack newItem;
					if (rand1 < ((int)((trommel.getLvl1Bonus(MineralNew.HAMMERS))*randMax))) {
						newItem = LoreMaterial.spawn(LoreMaterial.materialMap.get("civ:hammers"));
					} else if (rand1 < ((int)((trommel.getLvl1Bonus(MineralNew.EMERALD))*randMax))) {
						newItem = ItemManager.createItemStack(CivData.EMERALD, 1);
					} else if (rand1 < ((int)((trommel.getLvl1Bonus(MineralNew.DIAMOND))*randMax))) {
						newItem = ItemManager.createItemStack(CivData.DIAMOND, 1);
					} else if (rand1 < ((int)((trommel.getLvl1Bonus(MineralNew.GOLD))*randMax))) {
						newItem = ItemManager.createItemStack(CivData.GOLD_INGOT, 1);
					} else if (rand1 < ((int)((trommel.getLvl1Bonus(MineralNew.IRON))*randMax))) {
